...
\begin{defn}[Flag]
For a finite dimensional $\mathbb{C}$-vector space $V$, a \textbf{(partial) flag}\index{flag!of vector space} of $V$ of length $d \in \mathbb{N}_{>0}$ is an increasing sequence of subspaces of $V$:
...
The complete flag variety\index{complete flag variety} $\mathcal{F}$ is defined as 
\begin{equation*}
\begin{aligned}
  \mathcal{F}:=\;& \GL_n/B \\ 
...
The $B$-orbit $BwB/B$ is called the \textbf{Schubert cell}\index{Schubert cell}, denoted by $\Omcell_w$. Since 
...
$\overline{\Omcell}_w \subseteq \mathcal{F}$ is called the \textbf{Schubert variety}\index{Schubert variety}. It is well-known that
...
\subcaption{$\mathcal{F}$}
	...
	\end{minipage}
\caption[Orbit and fixed points of $\mathcal{F}$ and $\mathcal{F} \times \mathcal{F}$]{Orbit and fixed points}
...
\begin{defn}[Quiver]
A quiver\index{quiver} is a quadruple 
$$Q=(v(Q),a(Q),s,t)$$
where
...
A representation\index{quiver representation} of $Q$ consists of the following data:
\begin{itemize}[noitemsep,topsep=0pt]
...
We denote the category of representations of $Q$ by $\rep(Q)$\index{repQ@$\rep(Q)$}.
\end{defn}
...
Fix a quiver $Q$, a $Q$-vector space\index{Q-vector space@$Q$-vector space}\index{vector space with quiver partition|see{$Q$-vector space}} is a finite dimensional $\mathbb{C}$-vector space with a direct sum decomposition
$$V= \bigoplus_{i \in v(Q)} V_i.$$
The dimension vector\index{dimension vector!of $Q$-vector space} of a $Q$-vector space is defined as
...
\begin{defn}
The total dimension vector\index{dimension vector!total dimension vector} of a $Q$-vector space $V$ is defined as
...
For any quiver $Q$, dimension vector $\dimvec{d}$, fix the canonical $Q$-vector space $V= \bigoplus_{i \in v(Q)} V_i$, the space of representations\index{space of representations|see{$\Rep_{\dimvec{d}}(Q)$}}\index{Repd(Q)@$\Rep_{\dimvec{d}}(Q)$} with dimension vector $\dimvec{d}$ is defined as
\begin{equation*}
\begin{aligned}
...
For a quiver representation $V \in \rep(Q)$, a (partial) flag of $V$\index{flag!of quiver representation} is defined as an increasing sequence of subrepresentation of $V$, i.e.,
...
For any flag $F: 0 \subseteq M_1 \subseteq \ldots \subseteq M_d \subseteq V$, the (flag-type) dimension vector of $F$\index{dimension vector!of flag-type} is defined as
...
\begin{remark}
We have different ways to express $\ww \in \absgp{W}_{\abdimvec{d}}= S_{\abdimvec{d}}$\index{ww1@$\ww$}. For example, take $\abdimvec{d}=5$, $\ww \in S_5$ by
...
For $i \in \left\{ 1, \ldots , \abdimvec{d}-1 \right\}$, the simple reflection\index{simple reflection} is defined as
...
\MinWd =\left\{\parbox[h][][c]{1.5cm}{ \begin{tikzpicture}[scale=0.5]
 ...
 \end{array}
 $$
 \caption[Notations of the symmetric group calculus]{Collected notation in $(3,2)$-case}
...
\begin{table}[ht]
  \vspace{0cm}
    \centering  \includegraphics[width=15cm]{figures/table/table_basic_information.pdf}
      \caption[Different expressions of symmetric groups]{Basic information of $(1,2)$-case}  
...
and $\absgp{B}_{\abdimvec{d}}$, $\absgp{T}_{\abdimvec{d}}$, $\absgp{N}_{\abdimvec{d}}$ are corresponding standard Borel, torus and unipotent subgroups, respectively.\index{algebraic group!absolute algebraic group}
...
and $B_{\dimvec{d}}$, $T_{\dimvec{d}}$, $N_{\dimvec{d}}$ are corresponding standard Borel, torus and unipotent subgroups.\index{algebraic group!relative algebraic group}
...
\begin{table}[ht]
  \vspace{0cm}
    \centering  \includegraphics[width=15cm]{figures/table/table_Lie_alg.pdf}
      \caption[Lie algebras in different cases]{Examples of Lie algebras}
      \label{table:Lie_alg}        
\end{table}
\end{eg}
...
The absolute complete flag variety\index{flag variety!absolute complete flag variety} $\mathcal{F}_{\abdimvec{d}}$ is defined as 
\begin{align*}
...
The relative complete flag variety\index{flag variety!relative complete flag variety} $\mathcal{F}_{\dimvec{d}}$ is defined as 
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
$F_{\Id}$ is called the \textbf{standard flag}\index{flag!standard flag} of $V$.
...
For a quiver $Q$ with complete flag-type dimension vector $\ftdimvec{d}$, define\index{incidence variety}
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
$\St_{\dimvec{d}}$ is called the \textbf{Steinberg variety}\index{Steinberg variety}.
\end{defn}
...
\label{subsec:stratification_flag_variety}\index{stratification!of flag variety}
...
\begin{defn}[Twisted action]\index{twisted action}
...
\subsection{Stratifications: incidence varieties}\label{subsec:stratification:_incidence_variety}\index{stratification!of incidence variety}
...
\begin{defn}[Tangent space of $T$-fixed points]\index{tangent space!of $T$-fixed points}
