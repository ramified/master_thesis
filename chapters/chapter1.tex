...
Fix $n \geqslant 1$, we denote $\GL_n:=\GL_n(\mathbb{C})$, $B$, $T$, $N$, $W$ be the standard Borel subgroup, standard torus, unipotent subgroup and Weyl group, respectively, i.e.,
{
...
$F$ is called a complete flag if $\dim M_j = j$ for all $j$, otherwise $F$ is called a partial flag.
\end{defn}
...
$\mathcal{F}$ is a well-studied variety, and has many combinatorical properties. For example, from the well-known Bruhat decomposition,
\footnotemark 
\footnotetext{For the most time the formula does not depend on the lift of $w$, so we abuse the notation of $w \in N_{\GL_n}(T)/T$ and $\tilde{w} \in N_{\GL_n}(T)$.}
...
For other Schubert varieties, the structures are quite dedicate and far away from the scope of this master thesis. For example, most  Schubert varieties are not smooth.
...
\end{aligned}
\end{equation*}
We color pieces of $\OOmcell_{s}$ by blue, and $\OOmcell_{ts,s}$ by ink blue.
\end{eg}
...
A morphism $f:V \longrightarrow W$ is a collection of morphisms $f_i: V_i \longrightarrow W_i$ (for every $i \in Q_0$) which make the following diagram commute:
...
\[\begin{tikzcd}
	{V_{s(a)} } & {V_{t(a)}} \\
	{W_{s(a)} } & {W_{t(a)}}
	\arrow["{f_{s(a)}}"', from=1-1, to=2-1]
	\arrow["{V_a}", from=1-1, to=1-2]
	\arrow["{f_{t(a)}}", from=1-2, to=2-2]
	\arrow["{W_a}", from=2-1, to=2-2]
\end{tikzcd}\]

We denote $\rep(Q)$ as the category of representations of $Q$.
\end{defn}
\begin{eg}
...
On the contrary, given $\dimvec{d} \in \prod_{i \in Q_0} \mathbb{N}_{\geqslant 0}$, we can construct a canonical $Q$-vector space of dimension vector $\dimvec{d}$, as follows:
...
For quiver $Q: \textcolor{red}{i} \longrightarrow \textcolor{blue}{i'}$, $\dimvec{d}=(3,2)$, the canonical $Q$-vector space of dimension vector $\dimvec{d}$ is
\begin{equation*}
\begin{aligned}
  V=\;& V_i \oplus V_{i'} \\ 
...
\end{eg}
\begin{remark}
The flag-type dimension vector $\ftdimvec{d}$ can be viewed as a partition of set $\{1,\ldots, \abdimvec{d} \}$, i.e., a map
$$\ptt: \{1,\ldots, \abdimvec{d}  \} \longrightarrow Q_0$$
such that $\#\ptt^{-1}(i)=\dimvec{d}_i$.\footnote{The partition corresponding to $\ptt$ is$$\{1,\ldots, \abdimvec{d}\}=\bigsqcup_{i \in Q_0}\ptt^{-1}(i).$$} We color the set $\{1,\ldots, \abdimvec{d}  \}$ by the partition $\ptt$. In the Example \ref{eg:5-case-0},
{
\def\arraystretch{0.8}
$$\ftdimvec{d}= \begin{pmatrix}
3,2\\ 2,2 \\ 2,1 \\ 1,1 \\ 0,1
...
\end{defn}
We also denote $\ww_{\max} \in \absgp{W}_{\abdimvec{d}}$, $w_{\max} \in W_{\dimvec{d}}$ to be the longest elements in $\absgp{W}_{\abdimvec{d}}$ and $W_{\dimvec{d}}$, respectively. See Table \ref{table:Wgp_collected_notations} for a picture of $\ww_{\max}$ and $w_{\max}$.
...
\begin{fact}\label{fact:5-case-0.05}
Every element $W_{\dimvec{d}}\ww \in W_{\dimvec{d}}\setminus\absgp{W}_{\abdimvec{d}}$ corresponds to a partition on set $\left\{ 1, \ldots , \abdimvec{d} \right\}$ (of a given number partition $\dimvec{d}$), which corresponds to a flag-type dimension vector $\ftdimvec{d}$, i.e., an ordered set of points colored by the vertices of $Q$.
\end{fact}
...
\end{aligned}
\end{equation*}
The picture of $us_i$ is shown in Table \ref{table:Lie_alg}.
\end{ex}
...
\MinWd =\left\{\parbox[h][][c]{1.5cm}{ \begin{tikzpicture}[scale=0.5]
 ...
 \end{array}
 $$
 \caption{Collected notations in $(3,2)$-case}
 \label{table:Wgp_collected_notations}
 \end{table}
 }
 %调整表格。。。大难题
\end{eg}
...
\end{eg}
\section{Algebraic group and Lie algebra}\label{sec:algebraic_group_Lie_algebra}
...
We set
$$G_{\dimvec{d}}:= \bigoplus_{i \in Q_0}\GL(V_i)=\bigoplus_{i \in Q_0}\GL_{\dimvec{d}_i}(\mathbb{C}) \subseteq \absgp{G}_{\abdimvec{d}},$$
...
\begin{remark}
Fix $M \in \Rep_{\dimvec{d}}(Q)$, the \textbf{Springer fiber}
...
Natural defined varieties resemble burr puzzles, they have delicate structures and can be decomposed into relatively easy pieces. In this subsection, we will find stratifications of varieties introduced in Section \ref{sec:typical_variety}, and fix notations of orbits. We will also mention about their $T$-fixed points. These stratifications will give us a basis for the $K$-theory and cohomology theory in Chapter \ref{chap:Ktheory}, while those $T$-fixed points will give us another "basis" in Chapter \ref{chap:localization}.
