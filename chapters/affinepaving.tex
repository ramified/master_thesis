...
\begin{defn}[Sectional morphism]\index{sectional morphism}
...
\begin{lemma}\label{lem:affineADcase}
	Assume that $Q$ is an affine quiver of type $A$ or $D$, $M \in \rep(Q)$ is the \textbf{regular quasi-simple} representation\index{quasi-simple representation}, then the Grassmannian $\Grr(\Phi(M))$ has an affine paving.
\end{lemma}
\begin{proof}
