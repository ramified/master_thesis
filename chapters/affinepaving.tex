...
U(X_1,X) &\longrightarrow \Gr(X) \shorttimes \Gr(X_1/X) &&\oder U(X,X_S)\\
U(X,X_S) &\longrightarrow \Gr(X_S) \shorttimes \Gr(X/X_S) &&\oder U(X_S,-).\footnotemark \\
\end{aligned}
\end{equation*}
\footnotetext{$\Gr(X_S)$ is empty or a singleton, so is $U(X_S,-)$, no matter what representation is in the dash mark.}
When $X_1 \longrightarrow Y$ is not injective, we get
\begin{equation*}
\begin{aligned}
\Gr(Y) &\longrightarrow \Gr(X) \shorttimes \Gr(Y/X) &&\oder U(X,X_S)\\
U(X,X_S) &\longrightarrow \Gr(X_S) \shorttimes \Gr(X/X_S) &&\oder U(X_S,-).\\
\end{aligned}
\end{equation*}
