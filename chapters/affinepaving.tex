...
\begin{defn}[Partial flag variety]\label{def:pf}
For a quiver representation $X \in \rep(Q)$ and an integer $d \geqslant 1$, we denote
...
For a quiver representation $X \in \rep(Q)$ and an integer $d \geqslant 2$, we denote
...
\section{Dimension vector}In this section we define some varieties indexed by dimension vectors. Recall Definition \ref{def:Q-vs} and \ref{def:ftdv}. Notice that every module $T \in \Mod(\mathbb{C}Q/I)$ can be viewed as a representation of $Q$, so we automatically have a notion of dimension vector for $T$.
...
\[\begin{tikzcd}
	\Gralg{R}_{\ftdimvec{f}}(T) & \displaystyle\prod_{(i,r) \in v\left(Q_{d,(str)}\right)} \Gr_{\ftdimvec{f}_{i,r}}\big(T_{(i,r)}\big),
	\arrow[hook, from=1-1, to=1-2]
\end{tikzcd}\]
$\Gralg{R}_{\ftdimvec{f}}(T)$ is then endowed with the subspace topology and subvariety structure.
\end{remark}
...
For $d \geqslant 2$, the virtual arrows of the quiver $Q_{d,\str}$ is defined as a triple $\big(va(Q_{d,\str}),s,t\big)$, where
...
For two dimension vectors $\ftdimvec{f},\ftdimvec{g}$ of $R$, the Euler form $\left< \ftdimvec{f},\ftdimvec{g}\right>_R$ is defined by
...
Let $Q$ be a quiver without oriented cycles. A sectional morphism $f \in \Hom_{\mathbb{C}Q}(M,N)$ is called as a sectional mono if $f$ is injective; a sectional mono is called minimal if $f_t \circ \cdots \circ f_{i+1}:X_i \longrightarrow N$ are surjective for any $i \in \{1,2,\ldots,t \}$.
\end{defn}
...
Denote the set
$$\Grr(\Phi(Y))_{\ftdimvec{f},\ftdimvec{g}}:= \Psi^{-1}\Big(\Grr_{\ftdimvec{f}}(\Phi(X)) \times \Grr_{\ftdimvec{g}}(\Phi(S))\Big)$$
and
let $\Psi_{\ftdimvec{f},\ftdimvec{g}}$ be the map $\Psi$ restricted to $\Grr(\Phi(Y))_{\ftdimvec{f},\ftdimvec{g}}$, i.e.,
$$\Psi_{\ftdimvec{f},\ftdimvec{g}}: \Grr(\Phi(Y))_{\ftdimvec{f},\ftdimvec{g}} \longrightarrow \Grr_{\ftdimvec{f}}(\Phi(X)) \times \Grr_{\ftdimvec{g}}(\Phi(S)).$$
...
\begin{theorem}[Generalizes {\cite[Theorem 32]{irelli2019cell}}]\label{thm:main2}
	...
	$$\Img \Psi_{\ftdimvec{f},\ftdimvec{g}} = \bigg(\Grr_{\ftdimvec{f}}(\Phi(X)) \times \Grr_{\ftdimvec{g}}(\Phi(S)) \bigg) \setminus \bigg(\Grr_{\ftdimvec{f}}(\Phi(X_S)) \times \Grr_{\ftdimvec{g}-\dimv \Phi(S^X)}\left(\Phi(S/S^X)\right) \bigg)$$
	where 
	\begin{equation*}
	\begin{aligned}
...
$$f: \Spec \mathbb{C} \longrightarrow \Img \Psi_{\ftdimvec{f},\ftdimvec{g}}  \subseteq \Grr_{\ftdimvec{f}}(\Phi(X)) \shorttimes \Grr_{\ftdimvec{g}}(\Phi(S)) $$
...
\begin{enumerate}[1.]
\item Realise Grassmannians as representable functors, and replace $\mathbb{C}$-modules by $A$-modules;
\item Verify that $\Psi_{\ftdimvec{f},\ftdimvec{g}}^{-1} (\Spec A)$ is a $\Hom_A(\mathcal{W},\Phi(X)_A/\mathcal{V})$-torsor, where $$(\mathcal{V},\mathcal{W}) \in \Grr_{\ftdimvec{f}}(\Phi(X))(A) \shorttimes \Grr_{\ftdimvec{g}}(\Phi(S))(A)$$ corresponds to the immersion $\Spec A \hookrightarrow \Img \Psi_{\ftdimvec{f},\ftdimvec{g}}\,$;
\item Verify that $\Hom_A(\mathcal{W},\Phi(X)_A/\mathcal{V})$ is a vector bundle over $\Spec A$ of constant dimension $\left< \ftdimvec{f},\dimv \Phi(X)-\ftdimvec{g}\right>_R\,$;
\item Find a section of $\Psi_{\ftdimvec{f},\ftdimvec{g}}^{-1} (\Spec A) \longrightarrow \Spec A$, which is essentially the splitting $\theta$ in \cite[Lemma 22]{irelli2019cell}.
\end{enumerate}
\end{remark}
...
\begin{lemma}[{Follows \cite[Lemma 2.22]{maksimau2019flag}}]\label{lem:smallvecdim}
	Suppose that the underlying graph of $Q$ is a tree. For an indecomposable representation $M \in \rep(Q)$ with $\ord(M) \leqslant 2$, the variety $\Grr_{\ftdimvec{f}}(\Phi(M))$ is either empty or a direct product of some copies of $\mathbb{P}^1$. Especially, the partial flag variety $ \Grr_{\ftdimvec{f}}(\Phi(M))$ has an affine paving.
\end{lemma}
\begin{proof}\belowdisplayskip=-12pt
	For every $i \in v(Q)$, $\dim_\mathbb{C} M_i\leqslant 2$. Since $Q$ is a tree and $M$ is indecomposable, for every $b\in a(Q)$ satisfying $\dim_\mathbb{C} M_{s(b)}= \dim_\mathbb{C} M_{t(b)}=2$, the map $M_{s(b)} \longrightarrow M_{t(b)}$ is an isomorphism. Therefore, when $\Grr_{\ftdimvec{f}}(\Phi(M)) \neq \varnothing$,\footnote{This condition imposes very strong restrictions on $\ftdimvec{f}$.} we get the natural embedding
	$$\Grr_{\ftdimvec{f}}(\Phi(M)) \longrightarrow \prod_{\substack{i \in v(Q) \;s.t.\\ \dim_\mathbb{C} M_i=2 \\ \ftdimvec{f}_{(i,r)}=1 \text{ for some }r}} \mathbb{P}^1,$$\\[0.3cm]
	...
	\[\begin{tikzcd}
		\mathbb{C} & {\mathbb{C}^2} & {\mathbb{C}^2} & \mathbb{C} &[-0.5cm] {\text{and}} &[-0.5cm] {\mathbb{C}^2} & {\mathbb{C}^2.}
...
By the same technique one can show that, for Dynkin quiver $Q$ and any representation $M$ with $\displaystyle\max_{i \in v(Q)} \dim_\mathbb{C} M_i \leqslant 2$, the variety $\Grr_{\ftdimvec{f}}(\Phi(M))$ has an affine paving. This result does not depend on Claim \ref{claim:bigorder}.
\end{remark}
...
\begin{table}
\centering
\includegraphics[width=6cm]{figures/affine_paving/dimension_Ext.pdf}
\vspace{1mm}
\caption{}
\label{table:dim}
\end{table}
...
\begin{remark}
	...
	Luckily, we can avoid this bad situation by carefully choosing the minimal sectional mono $X \longrightarrow Y$. The minimal sectional monos I chose are presented in Figure \ref{fig:minisecmono}. In section \ref{appendix:proofcomplement} we will write down the induction process in detail for some examples.
\end{remark}
...
The exceptional cases are similar, but the discussion is a bit more complicated. Let us look at some examples. (We simplify the notations: $\Gr(M)$ as $\Grr_{\ftdimvec{f}}(\Phi(M))$, $U(M,N)$ as $\Grr_{\ftdimvec{f}}(\Phi(M)) \setminus \Grr_{\ftdimvec{f}}(\Phi(N))$, and we also ignore the dimension vectors.)
...
%https://tex.stackexchange.com/questions/233481/subcaption-defaulting-to-capital-letter-labels
		...
		\centering
			...
			\includegraphics[width=3.2cm]{figures/affine_paving/E8graypic1.pdf}
			\subcaption{}
			}
			\parbox[b]{.23\textwidth}{\centering
			\includegraphics[width=3.2cm]{figures/affine_paving/E8graypic2.pdf}
			\subcaption{}
			}
			\parbox[b]{.23\textwidth}{\centering
			\includegraphics[width=3.2cm]{figures/affine_paving/E8graypic3.pdf}
			\subcaption{}
			}
			\parbox[b]{.23\textwidth}{\centering
			\includegraphics[width=3.2cm]{figures/affine_paving/E8graypic4.pdf}
			\subcaption{}
			}
