...
The $B$-orbit $BwB/B$ is called the \textbf{Schubert cell}, denoted by $\Vcell_w$. Since 
...
$\overline{\Vcell}_w \subseteq \mathcal{F}$ is called the \textbf{Schubert variety}. It is well-known that
$$\overline{\Vcell}_w= \bigsqcup_{w' \leq w} \Vcell_w$$
as a set. Especially, for any $s\in W$ with $l(s)=1$, denote $P_s=B \sqcup BsB$,
$$\overline{\Vcell}_s= \Vcell_{\Id} \sqcup \Vcell_s = B/B \sqcup BsB/B = P_s/B \cong \mathbb{P}^1.$$
...
\subsubsection{$\mathcal{F} \times \mathcal{F}$}
...
Denote $\VVcell_{w'}:= \GL_n \cdot (F_{\Id}, F_{w'})$, then $\mathcal{F} \times \mathcal{F}= \sqcup_{w'} \VVcell_{w'}$. Moreover, by the orbit-stabilizer theorem, we get
$$\VVcell_{w'} \cong \GL_n/\left( B \cap w'B(w')^{-1}  \right)$$
...
$$(g_1,g_2,\underline{F}_{g,g'})=\underline{F}_{g_1g,g_2g'}.$$
...
\begin{equation*}
\begin{aligned}
  &\VVcell_{w,w'}:=\; (B \times B) \cdot \underline{F}_{w,w'} \subseteq \mathcal{F}\times \mathcal{F}\\ 
  &\mathcal{F}\times \mathcal{F}= \bigsqcup_{w, w'\in W} \VVcell_{w,w'} \qquad\VVcell_{w'}= \bigsqcup_{w\in W} \VVcell_{w,w'} \\
  & \VVcell_{w,w'} \cong B/(B \cap wBw^{-1}) \times B/(B \cap w'Bw'^{-1}) \cong \mathbb{A}^{l(w)+l(w')}
\end{aligned}
\end{equation*}
...
\GL_n              & \VVcell_{w'}              & \varnothing                                \\ \hline
B \times B         & \VVcell_{w,w'}            & \{F_{\Id,\Id} \}                           \\ \hline
...
Like $\mathcal{F}$, we also study the closure of $\VVcell_{w'}$ and $\VVcell_{w,w'}$ in special case. It can be shown that 
$$\overline{\VVcell}_{w'} = \bigsqcup_{x'\leq w'} \VVcell_{x'} \qquad 
\overline{\VVcell}_{w,w'} = \bigsqcup_{x \leq w, x'\leq w'} \VVcell_{x,x'}$$
as a set. Especially, for any $s \in W$ with $l(s)=1$,

\begin{equation*}
\begin{aligned}
 \overline{\VVcell}_{s} = \VVcell_{\Id} \sqcup \VVcell_{s} \cong\;& 
...
\begin{equation*}
\begin{aligned}
\overline{\VVcell}_{\Id,s} = \VVcell_{\Id,\Id} \sqcup \VVcell_{\Id,s} \cong\;& 
...
Now wee understand a lot about $\mathcal{F}$ and $\mathcal{F} \times \mathcal{F}$, and the whole process of analysis(investigations?) will be applied repeatedly in subsection ???. 
\section{Quiver}
...
$$V_{\ww,i}:= \left< e_{\ww(1)},\ldots e_{\ww(i)} \right> \subseteq V.$$
...
$$\mathfrak{r}_{\ww}:=\; \left\{ (f_a)_{a\in Q_1} \in \Rep_{\dimvec{d}}(Q) \;\middle| \;  f_a(V_{\ww,i} \cap V_{s(a)}) \subseteq V_{\ww,i} \right\},$$
\begin{equation*}
\begin{aligned}
