...
\[\begin{tikzcd}[column sep={2cm,between origins}, row sep=small]
	{\mathcal{F}} && {\makebox[10ex][l]{$\alpha_{g_1,g_2}^{*}\mathcal{F}=\alpha_{g_1}^{*}\alpha_{g_2}^{*}\mathcal{F}$}} & [2cm]{ \mathcal{F}_x} && {\mathcal{F}_{g_1g_2x}} \\
	&& {} & {} \\
...
From these examples we already see the difficulty of computing $K$-theories. Therefore, a series of properties of $K$-theories are definitely needed for computations. To state these properties, we need to define some factors of $K$-theory.
