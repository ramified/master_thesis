...
$$*: K^{G_{\dimvec{d}}}(\St_{\dimvec{d}}) \times K^{G_{\dimvec{d}}}(\St_{\dimvec{d}}) \longrightarrow K^{G_{\dimvec{d}}}(\St_{\dimvec{d}}).$$
\end{defn}
...
$$\divideontimes: K^{G_{\dimvec{d}}}(\St_{\dimvec{d}}) \times K^{G_{\dimvec{d}}}\left(\RRep_{\dimvec{d}}(Q)\right) \longrightarrow K^{G_{\dimvec{d}}}\left(\RRep_{\dimvec{d}}(Q)\right).$$
\end{defn}
...
\[\begin{tikzcd}
	...
	\arrow["\divideontimes", from=1-5, to=1-6]
	\arrow["\divideontimes", from=2-5, to=2-6]
	\arrow["\divideontimes", from=3-5, to=3-6]
...
$$*: K^{G_{\dimvec{d}}}\left(\RRep_{\dimvec{d}}(Q)\right) \times K^{G_{\dimvec{d}}}(\St_{\dimvec{d}}) \longrightarrow K^{G_{\dimvec{d}}}(\St_{\dimvec{d}}).$$
\end{defn}

\subsection{Convolution product formula}
???(add preimage before psi)

In this subsection, we prove the convolution product formula.
\begin{proposition}[Convolution product formula]
For $\ww$, $\ww'$, $\ww''$, $\ww''' \in \WWd$, we have
\begin{equation*}
\begin{aligned}
  \psi_{\ww''',\ww''} * \psi_{\ww',\ww}=\;& \delta_{\ww'',\ww'}\preimage{\Lambda}_{\ww'} \psi_{\ww''',\ww} \\ 
  \psi_{\ww'',\ww'} \divideontimes \psi_{\ww}=\;& \delta_{\ww',\ww}\preimage{\Lambda}_{\ww} \psi_{\ww''}. \\   
\end{aligned}
\end{equation*}
\end{proposition}
\begin{proof}
...
\paragraph*{\underline{Case 1.}}Assume $\ww'' \neq \ww'$, need to show $\psi_{\ww''',\ww''} * \psi_{\ww',\ww}=0$.\\[-3mm]

\paragraph*{\underline{Case 2.}}Assume $\ww' \neq \ww'$, need to show $\psi_{\ww'',\ww'} \divideontimes \psi_{\ww}=0$.\\[-3mm]

\paragraph*{\underline{Case 3.}}For $\ww$, $\ww'$, $\ww'' \in \WWd$, need to show that $$\psi_{\ww'',\ww'} * \psi_{\ww',\ww}=\preimage{\Lambda}_{\ww'}\psi_{\ww'',\ww}.$$

\paragraph*{\underline{Case 4.}}For $\ww$, $\ww' \in \WWd$, need to show that $$\psi_{\ww',\ww} * \psi_{\ww}=\preimage{\Lambda}_{\ww}\psi_{\ww'}.$$
\end{proof}
