...
\begin{tikzpicture}[node distance=2cm, 
  ...
  \node (indrep)     [process, below of=start]          {$\Flag(M)$\\ind representation $M$};
  \node (bigord)      [process, below of=indrep,yshift=-1cm]   {$\Flag(M)$\\$\ord(M)>2$};
  \node (smallord)     [process, left of=bigord,xshift=-1cm]   {$\Flag(M)$\\$\ord(M) \leqslant 2$};
...
\draw[->]   (start) --node [anchor=west,font=\ttfamily,xshift=1mm]{Theorem A} (indrep);
\draw[->]   (indrep) -- (bigord);
\draw[->]   (smallord) --node [anchor=west,font=\ttfamily,xshift=1mm]{Lemma} (stop);
  \draw[->]      ($ (indrep)!.5!(bigord) $) -| (smallord);
   \draw[->,red, dashed] ($ (bigord.south)+(\a,0)$) -- ++(0,-1) -- ++(5,0) --node[anchor=west,font=\ttfamily,xshift=1mm,align=left]
          {Theorem B\\\textcolor{red}{\phantom{irr mono}}} ++(0,3.8) -- ++ (-5,0) --              
       ($(bigord.north)+(\a,0)$);
   \draw[->,red, dashed] ($ (indrep)!.5!(bigord)+(\a,\b) $)-| ($ (smallord.north)+(\a,0) $);
  \end{tikzpicture}
\end{document}