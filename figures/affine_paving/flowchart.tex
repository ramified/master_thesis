...
\draw[->]   (start) --node [anchor=west,font=\ttfamily,xshift=1mm]{Theorem 8.1.2} (indrep);
\draw[->]   (indrep) -- (bigord);
\draw[->]   (smallord) --node [anchor=west,font=\ttfamily,xshift=1mm]{Lemma 8.2.2} (stop);
  \draw[->]      ($ (indrep)!.5!(bigord) $) -| (smallord);
   \draw[->,gray] ($ (bigord.south)+(\a,0)$) -- ++(0,-1) -- ++(5,0) --node[anchor=west,font=\ttfamily,xshift=1mm,align=left]
          {Theorem 8.1.3 \\ Grey:in Section 8.3} ++(0,3.8) -- ++ (-5,0) --              
       ($(bigord.north)+(\a,0)$);
